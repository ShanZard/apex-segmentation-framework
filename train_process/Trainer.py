...
class Trainer(object):
    ...
    def __init__(self, local_rank, model, 
                 loader,val_loader,out, max_epoch, stop_epoch=None, lr_gen=1e-3, 
                 lr_decrease_rate=0.95, interval_validate=None, batch_size=8, warmup_epoch=-1,half_train=False):
        ...
        self.best_epoch = -1
        self.lr_scheduler=torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_model,50,eta_min=1e-6,last_epoch=-1,verbose=False)
    ...
    def train_epoch(self):
        ...
        focal=FocalLoss(mode='multiclass').to(device)
        self.model,optim_model=amp.initialize(self.model,optim_model,opt_level='O1')
        ...
        for batch_idx, sampleS in tqdm.tqdm(
            ...
            outputchunk=torch.argmax(torch.softmax(output,dim=1),dim=1).float()
            outputchunk=torch.chunk(outputchunk,chunks=self.batch_size,dim=0)
            leng=len(outputchunk)
            gamma=0
            for i in range(leng-1):
                testimage=np.array(outputchunk[i].detach().cpu().numpy())
                mid=largest_component(testimage)
                gamma+=mid
            gamma=gamma/leng
            lossfocal=FocalLoss(mode='multiclass',gamma=gamma).to(device)
            lossfocal=focal(output,label)
            loss_seg = celoss(output, label).to(device)
            loss_seg=lossfocal+loss_seg
            ...
            # write train different network or freezn backbone parameter
                ...
                # self.writer.add_scalar('train/gamma_data', gamma_data, iteration)
                self.writer.add_scalar('train/gamma_data', facal_data, iteration)
        ...
        print('\n[Epoch: %d] lr:%f,  Average segLoss: %f, '
              'Execution time: %.5f' %
              (self.epoch, get_lr(optim_model), self.running_seg_loss, stop_time - start_time))
    ...
    def train(self):
        for epoch in tqdm.trange(self.epoch, self.max_epoch,
            ...
            # if (epoch+1) % 10 == 0:
            # #     lr_gen = self.lr_gen * self.lr_decrease_rate
